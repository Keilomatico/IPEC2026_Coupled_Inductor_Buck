...
\mathSymbol{Pout}{P_{out}}{Output Power}
...
\begin{figure}
  \begin{subfigure}[c]{\columnwidth}
      \centering
      \input{figures/waveform_negCoupling.tikz}
      \subcaption{Negative Coupling (\sbl{k} = -0.3)}
      \label{fig:waveform_negCoupling}
    \end{subfigure}
    \begin{subfigure}[c]{\columnwidth}
      \centering
      \input{figures/waveform_posCoupling.tikz}
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/DC_negCoupling.png}
  \hfill
  \vphantom{0.2mm}
  \includegraphics[width=\columnwidth]{figures/DC_posCoupling.png} \\
  \caption{DC-Flux for negative coupling (top) and positive coupling (bottom). With negative coupling, the two windings generate an opposing magnetomotive force resulting in a low flux and relatively uniform distribution. For positive coupling, the two magnetomotive forces are driving a flux in the same direction causing a large flux that only circulates between the two windings. This requires a much larger core to get the same losses.}
...
The converter operates over the entire range as expected. The efficiency is shown in figure \ref{fig:} which reaches its maximum of \qty{96.5}{\percent} at around \qty{550}{\W}, demonstrating the efficacy of the inductor structure.
