...
\mathSymbol{Ion}{I_{neg}}{Turn-on current}
...
ToDo: Operation with soft-switching! Some waveforms and schematic!
...
As mentioned before, for soft-switching \sbl{Ion} needs to be negative and generally needs to be below a certain value to guarantee a sufficiently short dead time which can be written as $\sbl{DeltaIleg} \geq \sbl{iout}+2\sbl{Ion}$. This is fulfilled for
\begin{equation}
    \sbl{fs} < \frac{\sbl{Vin}\sbl{Deff}}{2\sbl{Lself}(\sbl{iout}+2\sbl{Ion})} \Bigl (\frac{2}{1+\sbl{k}}(0.5-\sbl{Deff}) + \frac{1}{1-\sbl{k}} \Bigr ).
