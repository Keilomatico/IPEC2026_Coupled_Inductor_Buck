...
\begin{IEEEkeywords}
automotive 48 V to 12 V converter, coupled inductor, high power density, soft switching
...
\begin{figure}
    ...
    \caption{Peak-to-peak leg and output current ripple in dependency of the coupling factor for different duty cycles \sbl{D} normalized to $\frac{\sbl{Vin}}{\sbl{fs} \sbl{Lself}}$. \textit{Note:} For $\sbl{D}=0.15$ and $\sbl{D}=0.35$ the output current ripple is identical so the curves overlap.}
...
\section{Conclusion}
This paper introduced a novel, negatively coupled, single turn inductor design which enables very high power density and efficiency in a 2-phase buck converter for \qty{48}{\V} to \qty{12}{\V} conversion. The challenging demand of a low inductance at high phase current was solved by connecting all layers in parallel with a stepped conductor width to mitigate the effects of the proximity effect. By using dual air gaps, the current distribution was further improved resulting in a \qty{30}{\percent} reduction in copper losses compared to the standard single air gap design with non-stepped windings. \\
The effects of positive and negative coupling were evaluated in detail. It was shown that even though negative coupling exhibits higher leg and output current ripple, it allows a reduction in core area of \qty{50}{\percent} for cores where the limiting factor is the DC flux. Following the comparison of four different inductor geometries, the best design was implemented in an experimental prototype which achieves a power density of \qty{80}{\kW\per\l} (\qty{1300}{\W\per\cubic\inch}) for \qty{1}{\kW} and a peak efficiency of \qty{96.3}{\percent}, demonstrating the potential of the proposed inductor design.
